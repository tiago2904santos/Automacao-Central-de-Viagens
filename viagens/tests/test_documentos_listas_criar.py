...
class DocumentosListasCriarTests(TestCase):
    def test_listas_documentos_retornam_200(self) -> None:
        self.assertEqual(self.client.get(reverse("justificativas_list")).status_code, 200)
    ...
    def test_justificativa_criar_get_e_post_criam_base_e_redirecionam(self) -> None:
        base_oficios = Oficio.objects.count()

        response_get = self.client.get(reverse("justificativa_criar"))
        self.assertEqual(response_get.status_code, 302)
        oficio_get = Oficio.objects.order_by("-id").first()
        self.assertIsNotNone(oficio_get)
        self.assertEqual(Oficio.objects.count(), base_oficios + 1)
        self.assertIn(reverse("justificativa_form", args=[oficio_get.id]), response_get.url)

        response_post = self.client.post(reverse("justificativa_criar"))
        self.assertEqual(response_post.status_code, 302)
        oficio_post = Oficio.objects.order_by("-id").first()
        self.assertIsNotNone(oficio_post)
        self.assertEqual(Oficio.objects.count(), base_oficios + 2)
        self.assertIn(reverse("justificativa_form", args=[oficio_post.id]), response_post.url)
    ...
    def test_links_de_editar_apontam_para_rotas_corretas(self) -> None:
        ...
        oficio_just = Oficio.objects.create(justificativa_texto="Justificativa de teste")
        response_just = self.client.get(f"{reverse('justificativas_list')}?status=completas")
        self.assertEqual(response_just.status_code, 200)
        self.assertContains(response_just, reverse("justificativa_form", args=[oficio_just.id]))
